...
def parse_args():
    ...
    parser.add_argument('--exp_dir', type=str, default='./scenarioII/provider', help='experiment root')
...
def main(args):
    ...
    try:
        checkpoint = torch.load(str(exp_dir) + '/best_model_m_{}.pth'.format(args.num_group))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        num_correct = 0
        num_total = 0
        for batch_id, (points, target, indices) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            # print(points.shape)
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            if (acc >= best_acc):
                savepath = str(exp_dir) + '/best_model_m_{}.pth'.format(args.num_group)
